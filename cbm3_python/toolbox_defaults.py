...
def get_cbm_executable_dir():
    return os.path.join(get_install_path(), "Admin", "Executables")
...
def get_install_path():
    ...
    appdata_dir = os.path.join(
        os.getenv("LOCALAPPDATA"), "Programs", "Operational-Scale CBM-CFS3"
    )

    use_program_files_dir = os.path.exists(
        program_files_dir
    ) and os.path.exists(os.path.join(program_files_dir, "Toolbox.exe"))
    use_appdata_dir = os.path.exists(appdata_dir) and os.path.exists(
        os.path.join(appdata_dir, "Toolbox.exe")
    )
