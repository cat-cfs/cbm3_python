...
class Simulator(object):
    ...
    def CreateMakelistFiles(self):
        ...
        cmd = (
            '"'
            + os.path.join(self.toolboxPath, r"createMakelistFiles.exe")
            + '" '
            + str(self.simID)
        )
    ...
    def loadMakelistSVLS(self):
        ...
        cmd = (
            '"'
            + os.path.join(self.toolboxPath, r"MakelistSVLLoader.exe")
            + '" '
            + str(self.simID)
        )
        loghelper.get_logger().info("Command line: " + cmd)
    ...
    def CreateCBMFiles(self, save_svl_by_timestep=False):
        ...
        cmd = (
            '"'
            + os.path.join(self.toolboxPath, r"createCBMFiles.exe")
            + '" '
            + str(self.simID)
        )
        loghelper.get_logger().info("Command line: " + cmd)
        ...
        inf = open(os.path.join(cbm_run_input_dir, "indicate.inf"), "w")
        inf.write("0\n")
        inf.flush()
        inf.close()
    ...
    def LoadCBMResults(self, output_path=None):
        ...
        cmd = (
            '"'
            + os.path.join(self.toolboxPath, r"LoaderCL.exe")
            + '" '
            + str(self.simID)
            + ' "'
            + results_path
    ...
    def DumpMakelistSVLs(self):
        ...
        cmd = (
            '"'
            + os.path.join(self.toolboxPath, r"DumpMakelistSVL.exe")
            + '" "'
            + str(self.simID)
            + ' "'
        )
