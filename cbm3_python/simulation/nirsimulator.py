...
import cbm3_python.simulation.nir_sql.afforestation_fixes \
    as afforestation_fixes
import cbm3_python.simulation.nir_sql.unmanaged_forest_fixes \
    as unmanaged_forest_fixes
...
class NIRSimulator(object):
    ...
    def __init__(self, config, base_projects):
        self.config = config
        self.base_projects = {
            x["project_prefix"]: x for x in base_projects}
    ...
    def get_local_results_path(self, project_prefix):
        return os.path.join(
            self.get_local_project_dir(project_prefix),
            "results",
            self.config["local_results_format"].format(project_prefix))
    ...
    def copy_project_local(self, project_prefix):
        ...
        logging.info(
            "copying project database source: '{0}', dest: '{1}'"
