...
class CreateAccountingRules(object):
    ...
    def get_or_add_rule_type(self, rule_type):
        ...
        existing = self.projectAccessDb.Query(
            rule_query, [rule_type]
        ).fetchone()
        if existing:
            return existing.accountingruletypeid

        self.projectAccessDb.ExecuteQuery(
            "INSERT INTO tblaccountingruletype (name) VALUES (?)", [rule_type]
        )
        result = self.projectAccessDb.Query(rule_query, [rule_type])
        return result.fetchone().accountingruletypeid

    def get_or_add_rule_sets(
        self, category, rule_tracking_type, ru=None, spu=None
    ):
        get_rule_sets_query = """
        ...
        if ru is not None:
            get_rule_sets_query = " ".join(
                [get_rule_sets_query, "AND spu.defaultspuid = ?"]
            )
            add_rule_sets_query = " ".join(
                [add_rule_sets_query, "AND spu.defaultspuid = ?"]
            )
        ...
        if spu is not None:
            get_rule_sets_query = " ".join(
                [get_rule_sets_query, "AND rs.spuid = ?"]
            )
            add_rule_sets_query = " ".join(
                [add_rule_sets_query, "AND spu.spuid = ?"]
            )
            get_query_params.append(spu)
            add_query_params.append(spu)

        existing = self.projectAccessDb.Query(
            get_rule_sets_query, get_query_params
        ).fetchall()
        if existing:
            return existing

        self.projectAccessDb.ExecuteQuery(
            add_rule_sets_query, add_query_params
        )
        result = self.projectAccessDb.Query(
            get_rule_sets_query, get_query_params
        )
    ...
    def create_accounting_rules(self):
        ...
        self.projectAccessDb.ExecuteMany(
            ...
            [
                ...
                ["total_biomass"],
                ["total_eco"],
                ["years_since_last_pass_disturbance"],
                ["ag_biomass_last_pass_disturbance"],
                ["total_biomass_last_pass_disturbance"],
                ["total_eco_last_pass_disturbance"],
            ],
        )
        ...
        with open(self.dist_classes_path) as dist_class_file:
            ...
            self.projectAccessDb.ExecuteMany(
                ...
                """,
                [
                    [row["DefaultDistTypeID"], row["Category"]]
                    for row in reader
                ],
            )
        ...
        with open(self.dist_rules_path) as csvfile:
            ...
            for row in reader:
                ...
                spu = row.get("SPUID")
                if spu:
                    rule_sets = self.get_or_add_rule_sets(
                        dist_class, rule_tracking_type, spu=spu
                    )
                elif ru:
                    rule_sets = self.get_or_add_rule_sets(
                        dist_class, rule_tracking_type, ru=ru
                    )
                else:
                    rule_sets = self.get_or_add_rule_sets(
                        dist_class, rule_tracking_type
                    )
                ...
                for rule_set in rule_sets:
                    value = (
                        self.get_makelist_value(rule_set.spuid)
                        if rule_value == "makelist"
                        else rule_value
                    )
                    self.projectAccessDb.ExecuteQuery(
                        add_rule_sql,
