...
def get_stock_changes_view(tfi):
    ...
    df["Delta Total Biomass"] = \
        tfi.GrossGrowth_AG + tfi.GrossGrowth_BG - \
        (tfi.BioCO2Emission + tfi.BioCH4Emission +
         tfi.BioCOEmission + tfi.SoftProduction +
         tfi.HardProduction + tfi.BiomassToSoil)
    df["Delta Total DOM"] = tfi.GrossGrowth_AG+tfi.GrossGrowth_BG - \
        (tfi.DOMCO2Emission + tfi.DOMCH4Emssion + tfi.DOMCOEmission +
         tfi.DOMProduction + tfi.BiomassToSoil)
    ...
    df["Net Growth"] = np.where(
        tfi.DistTypeID == 0,
        tfi.DeltaBiomass_AG + tfi.DeltaBiomass_BG, 0.0)
    df["Net Litterfall"] = np.where(
        tfi.DistTypeID == 0,
        tfi.DeltaDOM, 0.0)
    df["Total Litterfall"] = np.where(
        tfi.DistTypeID == 0,
        tfi.BiomassToSoil, 0.0)
    df["Decomposition Releases"] = np.where(
        tfi.DistTypeID == 0,
        (tfi.VFastAGToAir + tfi.VFastBGToAir + tfi.FastAGToAir +
         tfi.FastBGToAir + tfi.MediumToAir + tfi.SlowAGToAir +
         tfi.SlowBGToAir + tfi.SWStemSnagToAir + tfi.SWBranchSnagToAir +
         tfi.HWStemSnagToAir + tfi.HWBranchSnagToAir + tfi.BlackCarbonToAir +
         tfi.PeatToAir), 0.0)
    ...
    df["Deadwood"] = (
        tfi.FastAGToAir + tfi.FastBGToAir + tfi.MediumToAir +
        tfi.SWStemSnagToAir + tfi.SWBranchSnagToAir + tfi.HWStemSnagToAir +
    ...
    df["Biomass To DOM"] = \
        tfi.MerchLitterInput + tfi.FolLitterInput + tfi.OthLitterInput + \
        tfi.CoarseLitterInput + tfi.FineLitterInput
