...
def mdb_xls_import(
    mdb_xls_path,
    age_class_table_name,
    classifiers_table_name,
    disturbance_events_table_name,
    disturbance_types_table_name,
...
def get_sit_config(
    ...
    sit_config.config["mapping_config"]["nonforest"] = mapping["nonforest"]
    sit_config.config["mapping_config"]["species"] = mapping["species"]
    sit_config.config["mapping_config"]["spatial_units"] = mapping[
        "spatial_units"
    ]
    sit_config.config["mapping_config"]["disturbance_types"] = mapping[
        "disturbance_types"
    ]
...
class SITConfig(object):
    ...
    def map_disturbance_type(self, user, default):
        ...
        self.config["mapping_config"]["disturbance_types"][
            "disturbance_type_mapping"
        ].append({"user_dist_type": user, "default_dist_type": default})
    ...
    def map_admin_boundary(self, user, default):
        ...
        if (
            "admin_mapping"
            not in self.config["mapping_config"]["spatial_units"]
        ):
            self.config["mapping_config"]["spatial_units"][
                "admin_mapping"
            ] = []
        self.config["mapping_config"]["spatial_units"]["admin_mapping"].append(
            {"user_admin_boundary": user, "default_admin_boundary": default}
        )
    ...
    def map_eco_boundary(self, user, default):
        ...
        self.config["mapping_config"]["spatial_units"]["eco_mapping"].append(
            {"user_eco_boundary": user, "default_eco_boundary": default}
        )
    ...
    def map_species(self, user, default):
        ...
        self.config["mapping_config"]["species"]["species_mapping"].append(
            {"user_species": user, "default_species": default}
        )
    ...
    def map_nonforest(self, user, default):
        ...
        if (
            "nonforest_mapping"
            not in self.config["mapping_config"]["nonforest"]
        ):
            self.config["mapping_config"]["nonforest"][
                "nonforest_mapping"
            ] = []
        self.config["mapping_config"]["nonforest"]["nonforest_mapping"].append(
            {"user_nonforest_type": user, "default_nonforest_type": default}
        )
    ...
    def text_file_paths(
        ...
        ageclass_path,
        classifiers_path,
        disturbance_events_path,
        disturbance_types_path,
        inventory_path,
