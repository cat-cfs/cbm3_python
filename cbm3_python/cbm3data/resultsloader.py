...
from cbm3_python.cbm3data.access_templates import copy_rrdb_template
import logging
...
class ResultsLoader(object):
    ...
    def createIndexes(self, db):
        db.ExecuteQuery(
            "CREATE INDEX fluxind_spuid_index ON tblFluxIndicators (SPUID)")
        db.ExecuteQuery(
            "CREATE INDEX fluxind_kf3_index ON tblFluxIndicators (kf3)")
        db.ExecuteQuery(
            "CREATE INDEX fluxind_kf6_index ON tblFluxIndicators (kf6)")
        db.ExecuteQuery(
            "CREATE INDEX fluxind_TimeStep_index ON "
            "tblFluxIndicators (TimeStep)")
        db.ExecuteQuery(
            "CREATE INDEX fluxind_UserDefdClassSetID_index ON "
            "tblFluxIndicators (UserDefdClassSetID)")
        db.ExecuteQuery(
            "CREATE INDEX fluxind_landclass_index ON "
            "tblFluxIndicators (LandClassID)")
        ...
        db.ExecuteQuery(
            "CREATE INDEX distind_TimeStep_index "
            "ON tblDistIndicators (TimeStep)")
        db.ExecuteQuery(
            "CREATE INDEX distind_UserDefdClassSetID_index "
            "ON tblDistIndicators (UserDefdClassSetID)")
        db.ExecuteQuery(
            "CREATE INDEX distind_landclass_index "
            "ON tblDistIndicators (LandClassID)")

        db.ExecuteQuery(
            "CREATE UNIQUE INDEX distTypeID_index ON "
    ...
    def addAnnualProcessDistType(self, rrdb):
        rrdb.ExecuteQuery(
            "INSERT INTO tblDisturbanceType "
            "(DistTypeID, DistTypeName, DefaultDistTypeID) "
            "VALUES (0, 'unknown', 0)")

    def copyProjectTables(self, proj, rrdb_path):
        proj.ExecuteQuery(
            "SELECT * INTO tblSPU IN '{0}' FROM tblSPU"
            .format(rrdb_path))
        proj.ExecuteQuery(
            "SELECT * INTO tblAdminBoundary IN '{0}' FROM tblAdminBoundary"
            .format(rrdb_path))
        proj.ExecuteQuery(
            "SELECT * INTO tblEcoBoundary IN '{0}' FROM tblEcoBoundary"
            .format(rrdb_path))
        proj.ExecuteQuery(
            "SELECT * INTO tblDisturbanceType IN '{0}' FROM tblDisturbanceType"
            .format(rrdb_path))

    def copyAidbTables(self, aidb, rrdb_path):
        aidb.ExecuteQuery(
            "SELECT * INTO tblSPUDefault IN '{0}' "
            "FROM tblSPUDefault".format(rrdb_path))
        aidb.ExecuteQuery(
            "SELECT * INTO tblAdminBoundaryDefault IN '{0}' "
            "FROM tblAdminBoundaryDefault".format(rrdb_path))
        aidb.ExecuteQuery(
            "SELECT * INTO tblEcoBoundaryDefault IN '{0}' "
            "FROM tblEcoBoundaryDefault".format(rrdb_path))
        aidb.ExecuteQuery(
            "SELECT * INTO tblDisturbanceTypeDefault IN '{0}' "
            "FROM tblDisturbanceTypeDefault".format(rrdb_path))
...
SELECT
    [qryClassifierSetValueNames].[ClassifierSetID] AS ClassifierSetIDCroseTab
FROM
    (
        SELECT
        ...
        FROM tblClassifierSetValues INNER JOIN
            tblClassifierValues ON
                tblClassifierSetValues.[ClassifierID] =
                    tblClassifierValues.[ClassifierID]
        WHERE tblClassifierSetValues.ClassifierValueID =
            [tblClassifierValues].[ClassifierValueID]
    ) as qryClassifierSetValueNames
...
"""
        ...
        for row in result:
            ...
            if(cset_string in self.cset_dict):
                logging.info(
                    "duplicate cset {0} found in project".format(cset_string))
...
(
    AgeIndID, TimeStep, SPUID, AgeClassID, UserDefdClassSetID, LandClassID,
    kf2, kf3, kf4, kf5, kf6, Area, Biomass, DOM, AveAge
) VALUES (
    {AgeIndID}, {TimeStep}, {SPUID}, {AgeClassID}, {UserDefdClassSetID},
    {LandClassID}, {kf2}, {kf3}, {kf4}, {kf5}, {kf6}, {Area}, {Biomass},
    {DOM}, {AveAge}
)
"""
 ...
 {LandClassID}, {kf2}, {kf3}, {kf4}, {kf5}, {kf6}, {DistArea}, {DistProduct}
        ...
        for line in self.tokenize(distIndPath):
            ...
            qryFormatted = qry.format(
                ...
                kf5=int(line[18]),
                kf6=int(line[19]),
                DistArea=float(line[20]),
                DistProduct=float(line[21]))
...
(
    FluxIndicatorID, TimeStep, DistTypeID, SPUID, UserDefdClassSetID,
    ...
    CoarseLitterInput, FineLitterInput, VFastAGToAir, VFastBGToAir,
    FastAGToAir, FastBGToAir, MediumToAir, SlowAGToAir, SlowBGToAir,
    SWStemSnagToAir, SWBranchSnagToAir, HWStemSnagToAir, HWBranchSnagToAir,
    BlackCarbonToAir, PeatToAir, LandClassID, kf2, kf3, kf4, kf5, kf6,
    MerchToAir, FolToAir, OthToAir, SubMerchToAir, CoarseToAir, FineToAir,
    GrossGrowth_AG, GrossGrowth_BG
)
VALUES
(
    {FluxIndicatorID}, {TimeStep}, {DistTypeID}, {SPUID},
    {UserDefdClassSetID}, {CO2Production}, {CH4Production}, {COProduction},
    {BioCO2Emission}, {BioCH4Emission}, {BioCOEmission}, {DOMCO2Emission},
    {DOMCH4Emssion}, {DOMCOEmission}, {SoftProduction}, {HardProduction},
    {DOMProduction}, {DeltaBiomass_AG}, {DeltaBiomass_BG}, {DeltaDOM},
    {BiomassToSoil}, {MerchLitterInput}, {FolLitterInput}, {OthLitterInput},
    ...
    {BlackCarbonToAir}, {PeatToAir}, {LandClassID}, {kf2}, {kf3}, {kf4},
    {kf5}, {kf6}, {MerchToAir}, {FolToAir}, {OthToAir}, {SubMerchToAir},
    {CoarseToAir}, {FineToAir}, {GrossGrowth_AG}, {GrossGrowth_BG}
)"""
        ...
        for line in self.tokenize(fluxIndPath):
            ...
            qryFormatted = qry.format(
                ...
                UserDefdClassSetID=int(cset_id),
                CO2Production=float(line[20]),
                CH4Production=float(line[21]),
                COProduction=float(line[22]),
                BioCO2Emission=float(line[23]),
                BioCH4Emission=float(line[24]),
                BioCOEmission=float(line[25]),
                DOMCO2Emission=float(line[26]),
                DOMCH4Emssion=float(line[27]),
                DOMCOEmission=float(line[28]),
                SoftProduction=float(line[29]),
                HardProduction=float(line[30]),
                DOMProduction=float(line[31]),
                DeltaBiomass_AG=float(line[32]),
                DeltaBiomass_BG=float(line[33]),
                DeltaDOM=float(line[34]),
                BiomassToSoil=float(line[35]),
                MerchLitterInput=float(line[36]),
                FolLitterInput=float(line[37]),
                OthLitterInput=float(line[38]),
                SubMerchLitterInput=float(line[39]),
                CoarseLitterInput=float(line[40]),
                FineLitterInput=float(line[41]),
                VFastAGToAir=float(line[42]),
                VFastBGToAir=float(line[43]),
                FastAGToAir=float(line[44]),
                FastBGToAir=float(line[45]),
                MediumToAir=float(line[46]),
                SlowAGToAir=float(line[47]),
                SlowBGToAir=float(line[48]),
                SWStemSnagToAir=float(line[49]),
                SWBranchSnagToAir=float(line[50]),
                HWStemSnagToAir=float(line[51]),
                HWBranchSnagToAir=float(line[52]),
                BlackCarbonToAir=float(line[53]),
                PeatToAir=float(line[54]),
                LandClassID=float(line[14]),
                kf2=float(line[15]),
                kf3=float(line[16]),
                kf4=float(line[17]),
                kf5=float(line[18]),
                kf6=float(line[19]),
                MerchToAir=float(line[55]),
                FolToAir=float(line[56]),
                OthToAir=float(line[57]),
                SubMerchToAir=float(line[58]),
                CoarseToAir=float(line[59]),
                FineToAir=float(line[60]),
                GrossGrowth_AG=(
                    float(line[32]) + float(line[36]) + float(line[37]) +
                    float(line[38]) + float(line[39])
                    ) if distTypeId == 0 else 0,
                GrossGrowth_BG=(
                    float(line[33]) + float(line[40]) + float(line[41])
                    ) if distTypeId == 0 else 0)
...
(
    ...
    {BlackCarbon}, {Peat},
    {LandClassID}, {kf2}, {kf3}, {kf4}, {kf5}, {kf6},
    {SW_Merch}, {SW_Foliage}, {SW_Other}, {SW_subMerch}, {SW_Coarse},
    {SW_Fine}, {HW_Merch}, {HW_Foliage}, {HW_Other}, {HW_subMerch},
    {HW_Coarse}, {HW_Fine}
)"""
        ...
        for line in self.tokenize(poolIndPath):
            ...
            qryFormatted = qry.format(
                ...
                SPUID=int(line[2]),
                UserDefdClassSetID=int(cset_id),
                VFastAG=float(line[31]),
                VFastBG=float(line[32]),
                FastAG=float(line[33]),
                FastBG=float(line[34]),
                Medium=float(line[35]),
                SlowAG=float(line[36]),
                SlowBG=float(line[37]),
                SWStemSnag=float(line[38]),
                SWBranchSnag=float(line[39]),
                HWStemSnag=float(line[40]),
                HWBranchSnag=float(line[41]),
                BlackCarbon=float(line[42]),
                Peat=float(line[43]),
                LandClassID=float(line[13]),
                kf2=float(line[14]),
                kf3=float(line[15]),
                kf4=float(line[16]),
                kf5=float(line[17]),
                kf6=float(line[18]),
                SW_Merch=float(line[19]),
                SW_Foliage=float(line[20]),
                SW_Other=float(line[21]),
                SW_subMerch=float(line[22]),
                SW_Coarse=float(line[23]),
                SW_Fine=float(line[24]),
                HW_Merch=float(line[25]),
                HW_Foliage=float(line[26]),
                HW_Other=float(line[27]),
                HW_subMerch=float(line[28]),
                HW_Coarse=float(line[29]),
                HW_Fine=float(line[30]))
...
(PreDistAgeID, SPUID, DistTypeID, TimeStep, UserDefdClassSetID, LandClassID,
 kf2, kf3, kf4, kf5, kf6, PreDisturbanceAge, AreaDisturbed)
VALUES
({PreDistAgeID}, {SPUID}, {DistTypeID}, {TimeStep}, {UserDefdClassSetID},
 {LandClassID}, {kf2}, {kf3}, {kf4}, {kf5}, {kf6}, {PreDisturbanceAge},
 {AreaDisturbed})"""
