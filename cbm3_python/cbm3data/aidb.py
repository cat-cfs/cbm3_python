...
class AIDB(AccessDB):
    ...
    def InsertRowTo_tblStandInitialization(
        self,
        name,
        desc,
        author,
    ...
    def InsertRowTo_tblCBMRun(
        self, name, desc, author, inputDBID, cbmRunID, inputCBMRunID
    ):
        self.ExecuteQuery(
            """
    ...
    def AddProjectToAIDB(self, project, project_sim_id=None):
        ...
        nextStandInitializationID = (
            self.GetMaxID("tblStandInitialization", "StandInitializationID")
            + 1
        )
        ...
        project_tblRunTableRow = project.Query(
            "SELECT * FROM tblRunTable WHERE " "tblRunTable.RunID = ?",
            project_tblSimulationRow.RunID,
        ).fetchone()
        ...
        project_tblStandInitializationRow = project.Query(
            ...
            project_tblSimulationRow.StandInitID,
        ).fetchone()
        ...
        self.InsertRowTo_tblCBMRun(
            project_tblRunTableRow.Name,
            project_tblRunTableRow.Description,
            " ",
            nextInputDBID,
            nextCBMRunID,
            project_tblRunTableRow.RunID,
        )

        self.InsertRowTo_tblSimulation(
            project_tblSimulationRow.Name,
            project_tblSimulationRow.Description,
            " ",
            nextInputDBID,
            nextSimulationID,
            nextStandInitializationID,
            nextCBMRunID,
            project_tblSimulationRow.SimulationID,
        )
    ...
    def DeleteProjectsFromAIDB(self, simulation_id=None):
        ...
        for item in keys:
            self.ExecuteQuery(
                "DELETE FROM tblInputDB WHERE InputDBID = ?", item.InputDBID
            )
